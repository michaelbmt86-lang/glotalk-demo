...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "GET" && url.pathname === "/livekit-token") {
    ...
    const { AgentDispatchClient: ADC } = require("livekit-server-sdk");
    ...
    try {
      ...
      const dispatchClient = new ADC(
        LK_URL.replace('wss://', 'https://'),
        LK_KEY, LK_SECRET
      );
      await dispatchClient.createDispatch(room, "glotalk-translator");
      log("Agent dispatched to room: " + room);
    } catch(e) { log('Agent dispatch: ' + e.message); }

    const token = await at.toJwt();
    if (!global._bridgeStarted) global._bridgeStarted = new Set();
    if (!global._bridgeStarted.has(room)) { global._bridgeStarted.add(room); startTranslationBridge(room, LK_URL, LK_KEY, LK_SECRET, process.env.GEMINI_API_KEY).catch(e => log("Bridge: " + e.message)); }
...
const { startTranslationBridge } = require("./translation-bridge.js");
const wss = new WebSocketServer({ noServer: true });

server.on("upgrade", (req, socket, head) => {
  const url = new URL(req.url, "http://localhost");
  if (url.pathname !== "/gemini-ws") { socket.destroy(); return; }
  ...
  wss.handleUpgrade(req, socket, head, (clientWs) => {
    ...
    geminiWs.on("open", () => {
      ...
      clientWs.on("message", (data) => { if (geminiWs.readyState === WS.OPEN) geminiWs.send(data); });
      geminiWs.on("message", (data) => { if (clientWs.readyState === WS.OPEN) clientWs.send(data); });
