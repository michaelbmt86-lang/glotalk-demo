...
class TranslationRouter {
  ...
  onTrackSubscribed(track, publication, participant) {
    if (track.kind === 1) { // 1=audio in @livekit/rtc-node
...
async function startTranslationBridge(roomName, lkUrl, lkKey, lkSecret, geminiApiKey) {
  ...
  try {
    ...
    room.on(RoomEvent.TrackSubscribed, (track, pub, participant) => {
      log(`[Bridge] TrackSubscribed: ${participant.identity} kind:${track.kind}`);
      // 忽略 translator bot 自己的轨道，防止循环
    ...
    for (const [id, p] of room.remoteParticipants) {
      for (const pub of p.trackPublications.values()) {
        if (pub.kind === 1 && pub.track) { // 1=audio
          router.cachedTracks.set(id, pub.track);
          log(`[Bridge] 扫描到已有音频轨道: ${id}`);
        }
      }
    }
