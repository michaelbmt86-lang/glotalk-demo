...
ALIBABA_WS_URL = 'wss://ws-lkqvvixmif3z6g8q.ap-southeast-1.maas.aliyuncs.com/api-ws/v1/realtime?model=qwen3.5-livetranslate-flash-realtime'
    ...
    async def _receive_translation(self, ali_ws):
        async for msg in ali_ws:
            ...
            if ev_type in ('session.created', 'session.updated'):
                ...
                logger.info(f'[Bot] 翻译: {transcript}')
                if transcript and self.room:
